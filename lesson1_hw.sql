...
select avg(price) as AvgpriceKia from cars where model = 'kia';

# знайти середнє арифметичне цін для кожної марки машин окремо
select avg(price) as AvgPriceModel from cars group by model;

# підрахувати кількість кожної марки
select count(*)from cars group by model;

# знайти марку машин кількість яких найбільше
select model from cars group by model order by count(*) desc limit 1;  /* вирішано*/
...
select * from cars where price > (select avg(price) as AvgPriceModel from cars);
